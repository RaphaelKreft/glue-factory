...
from ..geometry.desc_losses import caps_window_loss, nll_loss, triplet_loss
...
class SuperPointV2(BaseModel):
    ...
    def _init(self, conf):
        ...
        if self.conf.backbone.name == "lcnn_hourglass":
            ...
            self.backbone = ResNetCustom(
                self.conf.backbone.blocks, last_activation=None
            )
            dense_feat_dim = 256
        ...
        if conf.has_detector:
            self.convPa = nn.Conv2d(
                dense_feat_dim, 256, kernel_size=3, stride=1, padding=1
            )
        ...
        if conf.has_descriptor:
            ...
            self.convDb = nn.Conv2d(
                256, conf.descriptor_dim, kernel_size=1, stride=1, padding=0
            )
        ...
        if self.conf.temperature == "learned":
            temperature = nn.Parameter(torch.tensor(50.0))
    ...
    def _forward(self, data):
        ...
        if self.conf.sparse_outputs and self.conf.has_detector:
            sparse_pred = self.get_sparse_outputs(
                ...
                orig_size=(
                    [*zip(data["height"], data["width"])] if "height" in data else None
                ),
            )
    ...
    def loss(self, pred, data):
        ...
        kp_loss0 = kp_ce_loss(
            ...
            data["keypoints0"],
            valid_kp0,
            valid_mask=(None if "valid_mask0" not in data else data["valid_mask0"]),
        )
        kp_loss1 = kp_ce_loss(
            pred["logits1"],
            data["keypoints1"],
            valid_kp1,
            valid_mask=(None if "valid_mask1" not in data else data["valid_mask1"]),
        )
    ...
    def metrics(self, pred, data):
        ...
        if "keypoints0" not in pred:
            sparse_pred0 = self.get_sparse_outputs(
                data["image0"],
                pred["dense_score0"],
                dense_desc=(pred["dense_desc0"] if self.conf.has_descriptor else None),
                orig_size=(
                    [*zip(data["height"], data["width"])] if "height" in data else None
                ),
            )
            sparse_pred1 = self.get_sparse_outputs(
                data["image1"],
                pred["dense_score1"],
                dense_desc=(pred["dense_desc1"] if self.conf.has_descriptor else None),
                orig_size=(
                    [*zip(data["height"], data["width"])] if "height" in data else None
                ),
            )
