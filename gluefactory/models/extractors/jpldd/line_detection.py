"""
Not learned line detection module from https://github.com/cvg/SOLD2/blob/main/sold2/model/line_detection.py
"""
...
from gluefactory.models.extractors.jpldd.utils import (
    convert_junc_predictions,
    line_map_to_segments,
)
...
class LineExtractor(object):
    ...
    def __init__(self, device, line_extractor_cfg):
        ...
        self.detect_thresh = (
            line_extractor_cfg.detect_thresh
        )  # in cfg file: detection_thresh: 0.0153846 # 1/65
    ...
    def __call__(self, junctions, line_heatmap, valid_mask=None):
        junc_np = convert_junc_predictions(
            junctions, self.grid_size, self.junc_detect_thresh, self.max_num_junctions
        )
...
class LineSegmentDetectionModule(object):
    ...
    def refine_heatmap_local(
        ...
        count_map = torch.zeros(heatmap.shape, dtype=torch.float, device=heatmap.device)
