...
from gluefactory.models.extractors.jpldd.metrics_lines import (
    compute_repeatability,
    match_segments_1_to_1,
)
...
def compute_loc_error(
    ...
    def loc_error_per_image(single_data, pred_prob):
        ...
        flat_indices = np.argpartition(pred_prob, -max_detections, axis=None)[
            -max_detections:
        ]
...
def compute_repeatability(
    ...
    for i in range(len(pred)):
        ...
        # Filter out predictions
        flat_indices = np.argpartition(cur_pred, -max_detections, axis=None)[
            -max_detections:
        ]
        ...
        flat_indices = np.argpartition(cur_warped_pred, -max_detections, axis=None)[
            -max_detections:
        ]
        ...
        warped_keypoints = keep_true_keypoints(
            warped_keypoints, np.linalg.inv(H_np), shape
        )
        ...
        norm = np.linalg.norm(
            true_warped_keypoints - warped_keypoints, ord=None, axis=2
        )
        count1 = 0
        count2 = 0
        if N2 != 0:
