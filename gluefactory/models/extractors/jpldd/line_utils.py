...
def merge_line_cluster(lines):
    ...
    cov = (
        1
        / 3
        * torch.einsum("ij,ik->ijk", points_bar, points_bar * weights)
        .sum(axis=0)
        .to(lines.device)
    )
    ...
    new_line = torch.stack(
        [cross + torch.amin(proj) * u, cross + torch.amax(proj) * u], axis=0
    )
    return new_line
...
def merge_lines(lines, thresh=5.0, overlap_thresh=0.0):
    ...
    if overlap_thresh == 0:
        ...
        close_endpoint = torch.linalg.norm(endpoints[:, None] - endpoints[None], axis=2)
        close_endpoint = (
